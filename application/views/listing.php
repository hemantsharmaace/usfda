...
<body  ng-controller="listController">
      ...
      <div class="collapse navbar-collapse" id="navbarResponsive">
        <ul class="navbar-nav ml-auto">
          <li class="nav-item active">
            <a style="font-size:28px" href="<?php echo base_url(); ?>">Home</a>
           </li>
  ...
  <!-- Jumbotron Header -->
    <header class="jumbotron my-2">
      <h1 class="display-3">Saved Ingredients</h1>
      </header>
    ...
    <div class="row text-center">
      ...
      <table class="table">
        <tr>
         <tr>
          <th colspan="2" ng-show="ingredients.length">Name</th>         
        </tr>         
        </tr>
        <tr class="animate-repeat" ng-repeat="item in ingredients">
          <td>{{ $index+1 }}</td>  
          <td>{{ item.name }}</td></tr>             
       <tr ng-show="ingredients.length"><td>Total Calories(per serving):</td><td>{{ totalCalories }} kcal</td></tr>
  <td ng-hide="ingredients.length" colspan="2">No results available!</td> </tr>
        </tr>
      </table>
